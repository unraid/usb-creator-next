...
int format_volume(_In_z_ LPCSTR vol, _In_ const format_params* params)
{
	...
	// Retry locking: Explorer/AV may briefly hold the volume open
	const DWORD kMaxTries = 100; // ~10s
	const DWORD kSleepMs  = 100;
	for (DWORD t = 0; t < kMaxTries; ++t)
	{
		printf("[FAT32FORMAT] Attempt # %d to lock device... \n", t);
		bRet = DeviceIoControl(hDevice, FSCTL_LOCK_VOLUME, NULL, 0, NULL, 0, &cbRet, NULL);
		if (bRet){
			printf("[FAT32FORMAT] Device locked!\n");
			break;
		}
		...
		{
			printf("[FAT32FORMAT] Device locked by another process, waiting for it to release... %d\n", err);
			Sleep(kSleepMs);
		}
		else
		{
			printf("[FAT32FORMAT] Failed to lock device: %d\n", err);
			break;
		}
...
int main(int argc, char* argv[])
{
	...
	if (strncmp(volume_argv, R"(\\?\Volume{)", 11) == 0)
	{
		...
		printf("[FAT32FORMAT] 1st if - Formatting volume...\n");
		format_volume(volume_guid, &p);
		printf("[FAT32FORMAT] 1st if - Formatting volume...[done]\n");
		printf("[FAT32FORMAT] 1st if - Settinging volume label...\n");
		strcat_s(volume_guid, "\\");
		printf("[FAT32FORMAT] 1st if - Setting volume label...[done]\n");
	...
	else
	{
		...
		printf("[FAT32FORMAT] 2nd if - Formatting volume...\n");
		format_volume(volume, &p);
		printf("[FAT32FORMAT] 2nd if - Formatting volume...[done]\n");
		printf("[FAT32FORMAT] 2nd if - Settinging volume label...\n");
		strcat_s(volume, "\\");
		printf("[FAT32FORMAT] 2nd if - Setting volume label...[done]\n");
