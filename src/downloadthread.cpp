...
DownloadThread::DownloadThread(const QByteArray &url,
                               const QByteArray &localfilename,
                               const QByteArray &expectedHash, QObject *parent)
    : QThread(parent), _startOffset(0), _lastDlTotal(0), _lastDlNow(0),
      _verifyTotal(0), _lastVerifyNow(0), _bytesWritten(0),
      _lastFailureOffset(0), _sectorsStart(-1), _url(url),
      _filename(localfilename), _expectedHash(expectedHash),
      _firstBlock(nullptr), _cancelled(false), _successful(false),
      _verifyEnabled(false), _cacheEnabled(false), _lastModified(0),
      _serverTime(0), _lastFailureTime(0), _inputBufferSize(0), _file(NULL),
      _writehash(OSLIST_HASH_ALGORITHM), _verifyhash(OSLIST_HASH_ALGORITHM),
      _cachehash(OSLIST_HASH_ALGORITHM) {
  if (!_curlCount)
...
void DownloadThread::setProxy(const QByteArray &proxy) { _proxy = proxy; }

QByteArray DownloadThread::proxy() { return _proxy; }

void DownloadThread::setUserAgent(const QByteArray &ua) { _useragent = ua; }

/* Curl write callback function, let it call the object oriented version */
size_t DownloadThread::_curl_write_callback(char *ptr, size_t size,
                                            size_t nmemb, void *userdata) {
  return static_cast<DownloadThread *>(userdata)->_writeData(ptr, size * nmemb);
}

int DownloadThread::_curl_xferinfo_callback(void *userdata, curl_off_t dltotal,
                                            curl_off_t dlnow,
                                            curl_off_t ultotal,
                                            curl_off_t ulnow) {
  return (static_cast<DownloadThread *>(userdata)->_progress(
              dltotal, dlnow, ultotal, ulnow) == false);
}

size_t DownloadThread::_curl_header_callback(void *ptr, size_t size,
                                             size_t nmemb, void *userdata) {
  int len = size * nmemb;
...
bool DownloadThread::_openAndPrepareDevice() {
  ...
  if (std::regex_match(_filename.constData(), m, windriveregex)) {
    ...
    if (!_nr.isEmpty()) {
      qDebug() << "Removing partition table from Windows drive #" << _nr << "("
               << _filename << ")";
      ...
      if (proc.exitCode()) {
        emit error(tr("Error running diskpart: %1")
  ...
  if (authopenresult == _file.authOpenCancelled) {
    ...
    QString msg =
        tr("Error running authopen to gain access to disk device '%1'")
            .arg(QString(_filename));
    msg += "<br>" +
           tr("Please verify if 'Unraid USB Creator' is allowed access to "
              "'removable volumes' in privacy settings (under 'files and "
              "folders' or alternatively give it 'full disk access').");
    QStringList args("x-apple.systempreferences:com.apple.preference.security?"
  ...
  if (!_file.open(QIODevice::ReadWrite | QIODevice::Unbuffered)) {
    ...
    if (fd != -1) {
      _file.open(fd, QIODevice::ReadWrite | QIODevice::Unbuffered,
                 QFileDevice::AutoCloseHandle);
    } else
#endif
    {
      emit error(
          tr("Cannot open storage device '%1'.").arg(QString(_filename)));
      return false;
    }
#endif
  }
  ...
  if (_filename.startsWith("/dev/")) {
    ...
    /* On some internal SD card readers CID/CSD is available, print it for
     * debugging purposes */
    QByteArray cid =
        _fileGetContentsTrimmed("/sys/block/" + devname + "/device/cid");
    QByteArray csd =
        _fileGetContentsTrimmed("/sys/block/" + devname + "/device/csd");
    if (!cid.isEmpty())
      qDebug() << "SD card CID:" << cid;
    if (!csd.isEmpty())
      qDebug() << "SD card CSD:" << csd;

    QByteArray discardmax = _fileGetContentsTrimmed("/sys/block/" + devname +
    ...
    if (discardmax.isEmpty() || discardmax == "0") {
      ...
      if (::ioctl(fd, BLKGETSIZE64, &devsize) == -1) {
        qDebug()
            << "Error getting device/sector size with BLKGETSIZE64 ioctl():"
            << strerror(errno);
      } else {
        ...
        if (::ioctl(fd, BLKDISCARD, &range) == -1) {
          ...
          qDebug() << "BLKDISCARD successful. Discarding took"
                   << _timer.elapsed() / 1000 << "seconds";
        }
      }
    }
  }
  ...
  if (knownsize > emptyMB.size()) {
    if (!_file.seek(knownsize - emptyMB.size()) ||
        !_file.write(emptyMB.data(), emptyMB.size()) || !_file.flush() ||
        ::fsync(_file.handle())) {
      emit error(tr(
          "Write error while trying to zero out last part of card.<br>"
  ...
  emptyMB.clear();
  _file.seek(0);
  qDebug() << "Done zeroing out start and end of drive. Took"
           << _timer.elapsed() / 1000 << "seconds";
#endif
...
void DownloadThread::run() {
  ...
  curl_easy_setopt(_c, CURLOPT_WRITEFUNCTION,
                   &DownloadThread::_curl_write_callback);
  curl_easy_setopt(_c, CURLOPT_WRITEDATA, this);
  curl_easy_setopt(_c, CURLOPT_XFERINFOFUNCTION,
                   &DownloadThread::_curl_xferinfo_callback);
  ...
  curl_easy_setopt(_c, CURLOPT_HEADERFUNCTION,
                   &DownloadThread::_curl_header_callback);
  ...
  if (_proxy.isEmpty()) {
    ...
    if (!proxyList.isEmpty()) {
      ...
      if (proxy.type() != proxy.NoProxy) {
        ...
        proxyUrl.setScheme(proxy.type() == proxy.Socks5Proxy ? "socks5h"
                                                             : "http");
...
attempt_download:
  ...
  /* Deal with badly configured HTTP servers that terminate the connection
     ...
     to slow SD card. And also reconnect if we detect from our end that transfer
     stalled for more than one minute */
  while (ret == CURLE_PARTIAL_FILE || ret == CURLE_OPERATION_TIMEDOUT ||
    ...
    qDebug() << "Resuming with HTTP range from offset:"
             << (qulonglong)_startOffset;
  ...
  default:
    ...
    if (!errorBuf[0])
      /* No detailed error message text provided, use standard text for libcurl
       * result code */
    ...
    char *ipstr;
    if (curl_easy_getinfo(_c, CURLINFO_PRIMARY_IP, &ipstr) == CURLE_OK &&
        ipstr && ipstr[0])
      errorMsg += QString(" - Server IP: ") + ipstr;

    _onDownloadError(tr("Please check your network connection and try again. Error downloading: %1").arg(errorMsg));
  }
}

bool DownloadThread::_shouldRetryFromStart(CURLcode ret, long httpCode) {
  // Check various conditions that indicate server doesn't support ranges
  return (ret == CURLE_RANGE_ERROR) ||
         (ret == CURLE_OK && httpCode == 200 && _startOffset > 0) ||
         (ret == CURLE_HTTP_RETURNED_ERROR &&
          (httpCode == 416 || httpCode == 501));
}
...
void DownloadThread::_resetForFullRestart() {
  ...
  /* Reset first-block handling so we properly skip writing the first block
   * again */
  if (_firstBlock) {
    qFreeAligned(_firstBlock);
    _firstBlock = nullptr;
  ...
  if (_cacheEnabled && _cachefile.isOpen())
    _cachefile.seek(0);
  // Cloudflare R2 does not directly support resuming downloads
  /* Reinitialize hashes */
...
size_t DownloadThread::_writeFile(const char *buf, size_t len) {
  ...
  QFuture<void> wh =
      QtConcurrent::run(&DownloadThread::_hashData, this, buf, len);
#else
  QFuture<void> wh =
      QtConcurrent::run(this, &DownloadThread::_hashData, buf, len);
#endif
  ...
  if ((size_t)written != len) {
    qDebug() << "Write error:" << _file.errorString()
             << "while writing len:" << len;
  }
...
bool DownloadThread::_progress(curl_off_t dltotal, curl_off_t dlnow,
...
QByteArray DownloadThread::data() { return _buf; }

bool DownloadThread::successfull() { return _successful; }

time_t DownloadThread::lastModified() { return _lastModified; }

time_t DownloadThread::serverTime() { return _serverTime; }
...
uint64_t DownloadThread::dlNow() { return _lastDlNow; }

uint64_t DownloadThread::dlTotal() { return _lastDlTotal; }

uint64_t DownloadThread::verifyNow() { return _lastVerifyNow; }

uint64_t DownloadThread::verifyTotal() { return _verifyTotal; }

uint64_t DownloadThread::bytesWritten() {
  if (_sectorsStart != -1)
    return qMin((uint64_t)(_sectorsWritten() - _sectorsStart) * 512,
                (uint64_t)_bytesWritten);
  else
    return _bytesWritten;
}

void DownloadThread::_onDownloadSuccess() { _writeComplete(); }
...
void DownloadThread::_onWriteError() {
  ...
  if (_file.errorCode() == ERROR_ACCESS_DENIED) {
    ...
    QSettings registry(
        ...
        "Defender Exploit Guard\\Controlled Folder Access",
        QSettings::Registry64Format);
    if (registry.value("EnableControlledFolderAccess").toInt() == 1) {
      msg += "<br>" + tr("Controlled Folder Access seems to be enabled. Please "
                         "add unraid-usb-creator.exe to the list of allowed "
                         "apps and try again.");
    }
...
void DownloadThread::_writeComplete() {
  ...
  if (!_expectedHash.isEmpty() && _expectedHash != computedHash) {
    ...
    if (_url.startsWith("file://") && _url.contains("lastdownload.cache")) {
      errorMsg =
          tr("Cached file is corrupt. SHA256 hash does not match expected "
             "value.<br>"
      ...
      errorMsg = tr("Local file is corrupt or has incorrect SHA256 hash.<br>"
                    "Expected: %1<br>Actual: %2")
                     .arg(QString(_expectedHash), QString(computedHash));
    } else {
      errorMsg =
          tr("Download appears to be corrupt. SHA256 hash does not match.<br>"
  ...
  if (!_file.flush()) {
    DownloadThread::_onDownloadError(
        tr("Error writing to storage (while flushing)"));
    _closeFiles();
    return;
  }
  ...
  if (::fsync(_file.handle()) != 0) {
    DownloadThread::_onDownloadError(
        tr("Error writing to storage (while fsync)"));
    _closeFiles();
    return;
  }
#endif
  ...
  if (_firstBlock) {
    ...
    if (!_file.write(_firstBlock, _firstBlockSize) || !_file.flush()) {
      ...
      DownloadThread::_onDownloadError(
          tr("Error writing first block (partition table)"));
      return;
    }
...
#ifdef Q_OS_LINUX
#ifndef QT_NO_DBUS
    /* mountutils only implemented unmount and not eject on Linux. Do so through
     * udisks2 */
    UDisks2Api udisks;
...
void DownloadThread::setVerifyEnabled(bool verify) { _verifyEnabled = verify; }

bool DownloadThread::isImage() { return true; }

void DownloadThread::setInputBufferSize(int len) { _inputBufferSize = len; }
...
void DownloadThread::setImageCustomization(
    const QByteArray &config, const QByteArray &cmdline,
    const QByteArray &firstrun, const QByteArray &cloudinit,
    const QByteArray &cloudInitNetwork, const QByteArray &initFormat,
    const QVariantMap &imgWriterSettings) {
  _config = config;
  _cmdline = cmdline;
  _firstrun = firstrun;
  ...
  try {
    ...
    if (_initFormat == "auto") {
      ...
      if (fat->fileExists("user-data")) {
        ...
        qDebug()
            << "user-data found on FAT partition. Assuming cloudinit support";
      } else if (issue.contains("pi-gen")) {
        /* If issue.txt mentions pi-gen, and there is no user-data file assume
         * it is a RPI OS flavor, and use the old systemd unit firstrun script
         * stuff */
        _initFormat = "systemd";
        qDebug()
            << "using firstrun script invoked by systemd customization method";
      } else {
        ...
        _initFormat = "cloudinit";
        qDebug() << "Unknown what customization method image supports. Falling "
                    "back to cloudinit";
      }
    }
